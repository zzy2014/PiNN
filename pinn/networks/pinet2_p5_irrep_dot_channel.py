...
class GCBlock(tf.keras.layers.Layer):
    def __init__(self, weighted: bool, pp_nodes, pi_nodes, ii_nodes, **kwargs):
        ...
        ii1_nodes[-1] = int(ii1_nodes[-1] * 3 + 8)
        ...
        self.pp5_layer = FFLayer(pp5_nodes, activation=None, use_bias=False)
        ...
        self.p5p1_layer = tf.keras.layers.Dense(
            pi_nodes[-1], use_bias=False, activation=None
        )
    ...
    def call(self, tensors):
        ...
        c = 64
        channels = [c, c, int(c/16), c, int(c/16)]
        i1_1, i1_2, i1_3, i1_4, i1_5 = tf.split(i1, channels, axis=-1)
        ...
        p5 = self.p5p1_layer(p5)
        dot1 = self.dot_layer1(tf.reshape(p5, (-1, 5, p5.shape[-1])))
        dot2 = self.dot_layer2(p3)
        p1t1 = dot1 + dot2 + p1
...
class PiNet2P5IrrepDotChannel(tf.keras.Model):
    ...
    def __init__(
        ...
        """
        super(PiNet2P5IrrepDotChannel, self).__init__()
